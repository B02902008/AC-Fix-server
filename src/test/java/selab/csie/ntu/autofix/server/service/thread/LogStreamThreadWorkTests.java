...
class LogStreamThreadWorkTests {
    ...
    void setup() {
        ...
        BDDMockito.willDoNothing().given(service).sendAutoFixLog(anyString(), anyString());
    ...
    void testRunLogFileNotExists() {
        new LogStreamThreadWork(-1, liveID, service).run();
        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);
        Mockito.verify(service, Mockito.times(1)).sendAutoFixLog(eq(liveID), captor.capture());
    ...
    void testRunSocketIDNotAlive() {
        new LogStreamThreadWork(0, deadID, service).run();
        Mockito.verify(service, Mockito.times(0)).sendAutoFixLog(anyString(), anyString());
    ...
    void testRunCorrectMessageSent() {
        new LogStreamThreadWork(0, liveID, service).run();
        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);
        Mockito.verify(service, Mockito.times(mockLog.size())).sendAutoFixLog(eq(liveID), captor.capture());
        Mockito.verify(service, Mockito.times(1)).sendWebSocketTerminate(eq(liveID));
